    ...
    <React.Fragment >
      ...
      <p class="h4 mb-5 font-weight-normal text-white">Booking Computer App</p></header>
       <article style={{paddingTop: 0, paddingBottom: 0}}>
         ...
         <p className='Textupperinput'>Provide Your account's email for which you want to reset your password!</p>
         <Formik
            initialValues={{email:''}}
         ...
         >
             ...
             <input name='email' type='email' className='InputboxEmail' placeholder='E-MAIL' value={values.email} onChange={handleChange} onBlur={handleBlur} required/>
             {errors.email && touched.email && (<label className="ForgotPassworderrors" >{errors.email}</label>)}
             
             <div className="buttondivison" 
             style={{display: "flex"}}>
               <input className="SendEmailInput" 
               type="submit" 
               value='SEND' 
               disabled={isSubmitting} style={{marginRight: '2px'}} />
